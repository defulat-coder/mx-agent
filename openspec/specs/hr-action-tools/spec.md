...
### Requirement: HR 审批状态一致性
主管和 HR 管理者审批请假或加班时 SHALL 严格校验动作、审批范围和当前状态。

#### Scenario: 重复或并发审批
- **WHEN** 同一申请被重复或并发审批
- **THEN** 仅第一次从待审批状态迁移的操作成功

#### Scenario: 自我审批
- **WHEN** 审批人尝试审批自己的请假或加班申请
- **THEN** 系统拒绝操作

#### Scenario: 请假通过
- **WHEN** 有足够对应年度与类型的假期余额且审批通过
- **THEN** 系统在同一事务内增加已用天数并扣减剩余天数；余额不足时申请保持待审批
